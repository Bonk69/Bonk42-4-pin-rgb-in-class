...
Button BU1(2, 2, 60, 1, 5, 3, 5, 6 );
Button BU2(4, 2, 61, 1, 5, 11, 9, 10);
Button BU3(7, 3, 62, 1, 5, 8, 13, 12 ); // this button is in the middle
    ...
    if (message == 0) {
      ...
      switch (BUTTONS[i]->Bcommand) { 
        case 0: //Note - used if its a keybord type of thingy
          MIDI.sendNoteOn(BUTTONS[i]->Bvalue, 127, BUTTONS[i]->Bchannel);
           switchLed(BUTTONS[i]->Bpin, false, bank, BUTTONS[i]->Br, BUTTONS[i]->Bg, BUTTONS[i]->Bb);
          break;
      ...
      switch (bank) { // each button class has a toggle variable for each bank to keep track of things. the following sets the correct toggle state sends midi and turns the correct led on/ off
         ...
         if (BUTTONS[i]->Btoggle1 == 0) {
            ...
            switchLed(BUTTONS[i]->Bpin, true, bank, BUTTONS[i]->Br, BUTTONS[i]->Bg, BUTTONS[i]->Bb);
 ...
 BUTTONS[i]->Btoggle = 2;
        for (int i = 0; i < NUMBER_BUTTONS; i = i + 1) {
          ...
          if (BUTTONS[i]->Btoggle2 == 1 ){
             switchLed(BUTTONS[i]->Bpin, true, 2, BUTTONS[i]->Br, BUTTONS[i]->Bg, BUTTONS[i]->Bb);
       }
          if (BUTTONS[i]->Btoggle2 == 0){
              switchLed(BUTTONS[i]->Bpin, false, 2, BUTTONS[i]->Br, BUTTONS[i]->Bg, BUTTONS[i]->Bb);
...
BUTTONS[i]->Btoggle = 3;
        for (int i = 0; i < NUMBER_BUTTONS; i = i + 1) {
          ...
          if (BUTTONS[i]->Btoggle3 == 1 ){
             switchLed(BUTTONS[i]->Bpin, true, 3, BUTTONS[i]->Br, BUTTONS[i]->Bg, BUTTONS[i]->Bb);
       }
          if (BUTTONS[i]->Btoggle3 == 0){
              switchLed(BUTTONS[i]->Bpin, false, 3, BUTTONS[i]->Br, BUTTONS[i]->Bg, BUTTONS[i]->Bb);
    ...
    BUTTONS[i]->Btoggle = 4;
        for (int i = 0; i < NUMBER_BUTTONS; i = i + 1) {
          ...
          if (BUTTONS[i]->Btoggle4 == 1 ){
             switchLed(BUTTONS[i]->Bpin, true, 4, BUTTONS[i]->Br, BUTTONS[i]->Bg, BUTTONS[i]->Bb);
       }
          if (BUTTONS[i]->Btoggle4 == 0){
              switchLed(BUTTONS[i]->Bpin, false, 4, BUTTONS[i]->Br, BUTTONS[i]->Bg, BUTTONS[i]->Bb);
    ...
    BUTTONS[i]->Btoggle = 1;
        for (int i = 0; i < NUMBER_BUTTONS; i = i + 1) {
          ...
          if (BUTTONS[i]->Btoggle1 == 1 ){
             switchLed(BUTTONS[i]->Bpin, true, 1, BUTTONS[i]->Br, BUTTONS[i]->Bg, BUTTONS[i]->Bb);
       }
          if (BUTTONS[i]->Btoggle1 == 0){
              switchLed(BUTTONS[i]->Bpin, false, 1, BUTTONS[i]->Br, BUTTONS[i]->Bg, BUTTONS[i]->Bb);
...
void switchLed(byte value, bool state, byte bank, byte r, byte g, byte b){
  switch (bank) {
  case 1: 
      switch (value) {
        case 2:
